...
test('loads each preset once with bounded concurrency when listing a large preset library', async () => {
    ...
    getPresetManager: async () => ({
      ...
      getCompletionPresetByName: async (name) => {
        ...
        return {
          name,
          extensions: {
            regex_scripts: [{ id: `${name}-regex` }],
            tavern_helper: { scripts: [{ type: 'script', name: `${name}-script` }] },
          },
        }
      },
    }),
  ...
  const items = await new TavernAdapter().listResources()
  assert.deepEqual(calls.sort(), names.slice().sort())
  assert.ok(maxActive <= 4)
  assert.equal(items.filter((item) => item.kind === 'regexPreset').length, names.length)
  assert.equal(items.filter((item) => item.kind === 'scriptPreset').length, names.length)
})
...
test('avatar skip never uploads; overwrite explicitly replaces only the named PNG', async () => {
  installContext()
  const calls = []
  globalThis.fetch = async (url, options) => {
  ...
  const adapter = new TavernAdapter()
  const avatar = new File(['png'], 'alice.png', { type: 'image/png' })
  assert.deepEqual(await adapter.importResource(avatar, 'userAvatar', 'skip', { targetName: 'alice.png' }), { status: 'skipped', name: 'alice.png' })
  assert.equal(calls.filter((call) => call.url === '/api/avatars/upload').length, 0)
  assert.deepEqual(await adapter.importResource(avatar, 'userAvatar', 'overwrite', { targetName: 'alice.png' }), { status: 'overwritten', name: 'alice.png' })
  const upload = calls.find((call) => call.url === '/api/avatars/upload')
  assert.equal(upload.options.body.get('overwrite_name'), 'alice.png')
  await assert.rejects(adapter.importResource(avatar, 'userAvatar', 'copy', { targetName: 'alice.png' }), /不能按同名创建副本/)
  assert.equal(calls.filter((call) => call.url === '/api/avatars/upload').length, 1)
})
...
test('exports global regex files with their own names instead of the scope label', async () => {
  installContext({
    extensionSettings: {
      regex: [
        { id: 'one', scriptName: '清理思维链', findRegex: '/one/g', replaceString: '' },
        { id: 'two', scriptName: '整理文本', findRegex: '/two/g', replaceString: '' },
      ],
    },
  })
  const adapter = new TavernAdapter()
  const items = (await adapter.listResources()).filter((item) => item.kind === 'regexGlobal')
  assert.deepEqual(items.map((item) => item.fileName), ['清理思维链.json', '整理文本.json'])

  for (const item of items) {
    const file = await adapter.exportResource(item)
    const payload = JSON.parse(await file.text())
    assert.equal(file.name, item.fileName)
    assert.equal(payload.sourceName, item.name)
    assert.equal(payload.global[0].scriptName, item.name)
  }
})
...
test('refreshes the Tavern persona list immediately after a successful import', async () => {
  ...
  const adapter = new TavernAdapter({
    refreshPersonas: async () => {
      refreshedName = context.powerUserSettings.personas['new.png']
    },
  })
  const file = new File([JSON.stringify({
    personas: { 'new.png': '即时可见' },
    persona_descriptions: { 'new.png': { description: '测试' } },
  })], 'personas.json')

  const result = await adapter.importUserPersona(file, 'skip')

  assert.equal(result.status, 'created')
  assert.equal(refreshedName, '即时可见')
})
