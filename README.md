...
> 当前页面扩展版本为 0.3.21。设备码仍通过 SRL 的 HTTPS 内存中继完成配对和控制消息；当 APK 与酒馆同在一台安卓设备且酒馆地址恰为 `http://127.0.0.1:8000`，用户可主动开启本机直传，文件字节不经过云端而在带随机令牌的临时本机端点传输。其余场景继续使用经 SHA-256 校验的限额分块中继；直传失败会自动回退，不会开放任意本机地址。
