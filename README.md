...
- 跨浏览器连接时，原 HTTPS 资源库始终保持为顶层页面并继续使用原 IndexedDB；酒馆只打开一个不保存资源的轻量中继窗口，避免浏览器第三方存储分区产生“另一份空资源库”。
...
> 页面扩展与服务端插件是两个组件。0.3.16 起，一键服务端安装脚本还会查找通过 Git 安装的现有页面扩展并执行安全的快进更新；页面扩展不存在、没有 Git 更新信息或存在本地修改时，脚本不会强行覆盖，而会给出明确的手动更新提示。
...
连接成功时只会出现一个写着“酒馆连接中继”的轻量小窗口，资源列表仍在原来的 HTTPS SRL 页面。如果新窗口里出现完整资源库或空资源库，立即关闭它并更新服务端插件。

手机不能填写 `127.0.0.1`，因为它指向手机自己。应填写运行酒馆电脑的局域网地址，例如 `http://192.168.1.10:8000`。公网使用必须配置 HTTPS、SillyTavern 登录和访问控制，不要直接裸露酒馆端口。

酒馆地址必须是用户平时能打开 SillyTavern 的完整地址，不是 SRL 资源库地址，也不要在域名后随意补 `:8000`。不同浏览器可以使用不同的酒馆登录会话；八位设备码是这次连接的短时通行证，加入后会立即换成随机长令牌，并继续要求核对六位确认码。
