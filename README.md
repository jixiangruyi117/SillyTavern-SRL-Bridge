...
> 当前页面扩展版本为 0.3.20。0.3.17 起移动端设备码连接 SRL 所在的 HTTPS 内存中继：酒馆页与原资源库页直接通过同一 HTTPS 会话交换分块，不要求打开或保留中继窗口，也不依赖 Android 标签页之间的 `window.opener`。资源内容只短暂存在限额内存队列，不写入服务器磁盘；等待连接 2 分钟过期，任一端离线约 30 分钟后清理。旧的本机服务端中继仅作为未部署新版 SRL 服务端时的兼容回退。
...
| SRL 打包为 APK 后连接酒馆 | 必须 | HTTPS 中继不需要 |
