...
适配 SillyTavern 1.18.x 的本地桥接扩展。支持角色卡、世界书、当前 API 类型预设、全局正则、快速回复组和酒馆主题与 SRL 双向传输。
...
- 酒馆助手脚本尚未接入；角色卡内嵌的正则和世界书继续跟随角色卡原文件传输。
- 当前直连固定使用正在打开酒馆的浏览器；不同浏览器需要后续启用服务端设备码中继。
